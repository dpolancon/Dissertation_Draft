...
% =========================================================================
% §1.4.4 — Stage S2: System-Level VECM Replication
% =========================================================================
...
% ============================================================
% §1.4 — Critical Replication: S0–S2 Results
% Chapter 1 — A Historical Trace of Capacity Utilization Measurements
% Dissertation: Diego Polanco
%
% WLM v3.1 calibrated: Critical Replication Results Mode + Econometrics Interpreter
% Epistemic discipline: findings are properties of the estimator at this site,
%   not structural claims, unless corroborated by S2/S3.
% Register: reportative throughout S0–S2.
%
% Recalibration edits applied (7/7):
%   [R1] §1.4.1 — mechanism statement before procedure
%   [R2] §1.4.3 — identification layer in non-replication
%   [R3] §1.4.3 — identification + reinterpretation after four-pairing table
%   [R4] §1.4.4 — measurement infrastructure reminder on exact p-values
%   [R5] §1.4.4 — reinterpretation of split verdict for CU series
%   [R6] §1.4.2 — proxy transparency statement for θ̂
%   [R7] — language softened: "confirms" → "indicates"
%
% Draft v2 | 2026-03-18
% ============================================================
...
% =========================================================================
% §1.4.1 — The Confinement Architecture
% =========================================================================

\subsection{The Confinement Architecture}
\label{subsec:confinement_architecture}

% [R1] Mechanism statement before procedure
The theoretical object under investigation is $\theta(\Lambda)$---the transformation elasticity of capital stock into productive capacity, which the accounting laws of \S\ref{sec:conceptual_framework} predict to be regime-dependent. The critical replication tests whether the ARDL methodology can recover this object under fixed-parameter estimation, and at what cost to identification.

The replication proceeds along four axes of a model uncertainty lattice: specification (S1), estimator (S2), institutional configuration (S3), and external restriction (Chile). The first two axes are methodological---they reduce researcher degrees of freedom. The second two are structural---they test whether the fixed-parameter estimation has suppressed detectable regime-dependence.

S0 is the lattice origin. It holds Shaikh's specification fixed---the ARDL(2,4), the three step dummies, the common deflator, the undisclosed PSS case---and asks what reproduces. S0 operates in the scalar $\theta$ world: one elasticity, one adjustment channel, one normalization. S1--S2 remain in that world; only S3 tests $\theta(\Lambda)$ via parametric change within the confined model.

The pipeline is: corporate accounting objects $\to$ common deflator $\to$ ARDL estimation $\to$ bounds admissibility $\to$ long-run multiplier recovery $\to$ capacity benchmark $\to$ utilization residual. Each stage inherits the output of the previous one. S3 inherits the utilization series implied by the confined S2 system---it does not re-estimate $\theta$ from scratch but tests whether the survivor of three prior stages is structurally stable across the Fordist/post-Fordist boundary.
...
\subsubsection{Variable Selection: Why NVA Replaces Shaikh's (Probable) GVA}
...
% [R7] "confirms" → "indicates" applied above

\begin{figure}[!ht]
	\centering
	% \includegraphics[width=\textwidth]{figures/fig_depreciation_psi.pdf}
	\caption{Depreciation share $s_t^D = D_t / \text{GVA}_t$ (left) and net-to-gross ratio $\psi_t = K_t^{N,R} / K_t^{G,R}$ (right), US corporate sector, 1947--2011. Linear trend overlays. The depreciation share deviations are small and stationary (flow ratio); the net-to-gross deviations are slow and persistent (stock ratio). The contrast motivates the pairing selection.}
...
% [R6] Proxy transparency statement
...
% =========================================================================
% §1.4.3 — S0: Faithful Replication and the Fragility Architecture
% =========================================================================

\subsection{S0: Faithful Replication and the Fragility Architecture}
\label{subsec:S0_results}


\subsubsection{Non-Replication}

No coefficient, fit statistic, or long-run multiplier from Shaikh's Appendix Table~6.7.14 replicates under current-release BEA data. The discrepancy is not marginal. Shaikh's capital stock coefficients (L0 through L4) are an order of magnitude larger than the replication's, and all are individually significant at the 1\% level; the replication finds significance only at L0 and L1. Shaikh's RMSE (0.016) is roughly half the replication's (0.031). The long-run elasticity $\theta$ is in the right neighborhood---0.661 (Shaikh) versus 0.745 (replication, no trend)---but the path to it is entirely different: different autoregressive structure, different denominator, different dummy effects.

The most likely sources are data release differences (the BEA's comprehensive revisions, especially the 2013 R\&D capitalization), Shaikh's undocumented variable definitions and deflator choice, and the unreported initial capital stock value. These are reproducibility failures, not specification failures---they concern the inputs to the method, not the method itself.

% [R2] Identification layer
The non-replication concerns point estimates, not the existence of the cointegrating relationship. Whether output and capital maintain a long-run equilibrium under current-release data is a separate question, tested in the four-pairing exercise and the PSS bounds test below. The non-replication alone is a narrow finding. Its contribution begins when the question shifts from \emph{what doesn't replicate} to \emph{whether the method is robust to the things that changed}.
...
\subsubsection{The Four-Pairing Exercise}
...
The table partitions into two blocks governed by the capital stock concept, not the output measure. The gross~$K$ block ($1 - \Sigma\gamma \approx 0.11$--$0.13$) shows $\theta$ dropping approximately 25\% upon trend inclusion, with $\pi_y$ flipping from negative-significant to positive-insignificant. The net~$K$ block ($1 - \Sigma\gamma \approx 0.05$--$0.09$) shows $\theta$ collapsing approximately 85\%, with $\pi_y$ retaining its sign but losing all statistical significance. Within either block, switching between GVA and NVA moves $\theta$ by less than three percentage points.

% [R3] Identification + reinterpretation sentence
Interpreted through the measurement framework, these results suggest that the ARDL long-run multiplier is not identified as a stable structural parameter at this estimation site. It is a specification-contingent projection through a near-zero denominator, whose severity is predictable from the capital stock concept used. The fragility gradient---the monotonic relationship between $1 - \Sigma\gamma$ and the magnitude of $\theta$ instability under trend inclusion---is the core empirical finding of the four-pairing exercise.


\begin{figure}[!ht]
	\centering
	% \includegraphics[width=\textwidth]{figures/cu_comparison_8series.pdf}
	\caption{Implicit capacity utilization: 8 pairings versus benchmarks, US corporate sector, 1947--2011. Left: No trend. Right: With trend. Blue family: Gross~$K$ (GVA and NVA). Orange family: Net~$K$ (GVA and NVA). Dark red: Shaikh $u_K$ (benchmark). Black: FRB industrial CU. The $K^G$ band is tight ($\sim$10pp); the $K^N$ band drifts upward past 120\%.}
...
A trend term competes with $\ln K$ for the same upward-trending variation in a postwar sample where capital grows monotonically. The trend absorbs part of what the autoregressive lags were carrying, slightly reducing $\Sigma\gamma$---and therefore slightly increasing the denominator. But the multiplier is a hyperbolic function of the denominator: a small perturbation near zero produces a large change in the ratio. The insignificant trend carries no information; it merely reshuffles weight between collinear regressors. A structural parameter should be invariant to such a reshuffling. $\theta$ is not, because the persistence of output amplifies the specification perturbation into a qualitative change in the long-run projection.

The capital stock concept determines the severity because the net-to-gross ratio $\psi_t$ contaminates the autoregressive dynamics when net~$K$ is used on the right-hand side, pushing $\Sigma\gamma$ closer to unity and shrinking the denominator further. The persistence amplification documented here is the econometric counterpart of the long-memory amplification that the accounting laws predict when $\theta < 1$: in the over-mechanization regime, the output--capital system has a weak attractor and long memory, and the ARDL's denominator sits close to zero as a direct consequence.

A note on interpretation is warranted. This persistence amplification is a property of the ARDL estimator at this site. Whether it reflects genuine long-memory dynamics in the output--capital relationship or an artifact of near-unit-root estimation cannot be resolved within the ARDL framework alone. The VECM estimator in S2 recovers $\theta$ as an eigenvector of a reduced-rank regression---a fundamentally different approach that does not pass through the near-zero divisor. If the eigenvector estimate is stable where the ARDL estimate is fragile, the instability is method-induced. If it persists, the instability is structural.


\subsubsection{Identification Issues}

Three properties of Shaikh's specification compound the fragility. First, the step dummies inside the cointegrating equation fall outside all five PSS cases. The bounds test critical values were simulated under DGPs without restricted dummies; inference under Shaikh's specification is formally invalid. Second, the ECT normalization $\hat{u}_t = \exp(\text{ECT}_t - \overline{\text{ECT}})$ imposes that average capacity utilization equals unity over the sample---a normalization assumption, not an estimation result, which Shaikh does not document. Third, computing long-run multipliers for step dummies is coherent only if they represent permanent regime shifts, but permanent step dummies inside the ECT produce visible discontinuities absent from Shaikh's published plots.


\subsubsection{What S0 Establishes}

S0 documents a fragility architecture that persists across every specification and variable pairing tested. The non-replication forces the question; the four-pairing exercise answers it. The method's core pathologies---the persistence amplification of specification choices through the near-zero denominator, the trend sensitivity, the capital-concept regime---are structural properties of the ARDL long-run multiplier at this estimation site, not artifacts of Shaikh's particular data release or variable definitions.

Three questions remain open, all traceable to the fact that Shaikh's ARDL(2,4) was held fixed throughout: (O1)~is the specification arbitrary? (O2)~which PSS case governs the bounds test? (O3)~does the normalization assumption bias the utilization series? S1 addresses O1 and O2 by releasing the specification axis.


% =========================================================================
% §1.4.4 — S1: Specification Confinement
% =========================================================================

\subsection{S1: Specification Confinement}
\label{subsec:S1_results}


\subsubsection{The Specification Search Shaikh Skipped}

The PSS bounds testing framework presupposes a specification search over two dimensions: the lag order $(p, q)$ and the deterministic case (Cases~1--5). Shaikh's ARDL(2,4) appears without a reported information criterion, without a specification grid, and without a stated PSS case. S1 releases this axis.

The choice of information criterion carries asymmetric risk. Over-specification (AIC's bias) wastes degrees of freedom but preserves clean residuals---the bounds test retains validity. Under-specification (BIC's bias) leaves residual serial correlation, which inflates the $F$-statistic and invalidates the PSS critical values. AIC's failure mode is imprecision; BIC's is invalid inference. For a test whose central question is whether a cointegrating relationship exists, the conservative choice is the criterion that preserves the test's validity.


\subsubsection{IC Consensus}
...
\begin{figure}[!ht]
	...
	\caption{S1 specification lattice. (a)~Admissible cloud in the fit-complexity plane ($-2\log L$ versus $k$); Pareto frontier in dark red; Shaikh's ARDL(2,4) marked with blue asterisk---dominated. (b)~IC tangency points: AIC, BIC, ICOMP cluster at $k \approx 6$. (c)~Informational domain $\hat{F}^{0.20}$: 15 specifications within 20\% of the frontier, $\theta \in [0.40,\, 0.71]$. (d)~Utilization band from frontier specifications: median with 10th/90th percentile, versus Shaikh $u_K$ and FRB benchmarks.}
	\label{fig:S1_composite}
\end{figure}


\subsubsection{Bounds Test Evidence}

% [R4] Measurement infrastructure reminder
The PSS bounds test is evaluated at exact finite-sample critical values conditioned on $T = 65$, using $R = 40{,}000$ simulation draws. This matters because asymptotic PSS critical values---the default in most software---are too permissive for this sample size; the finite-sample penalty makes the Case~2 rejection more conservative, and therefore more credible, than it would appear under asymptotic tables.


\begin{table}[!ht]
	\centering
	\small
	\caption{PSS (2001) bounds test for cointegration, Cases~I--V. ARDL(1,3), no dummies.}
	\label{tab:S1_bounds_nodummy}
	\begin{tabular}{lrrrr}
		\toprule
		& \multicolumn{2}{c}{$F$-bounds test} & \multicolumn{2}{c}{$t$-bounds test} \\
		\cmidrule(lr){2-3}\cmidrule(lr){4-5}
		Case & $F$ & $p$ & $t$ & $p$ \\
		\midrule
		Case~1 & $5.19$ & $0.023$ & $+2.02$ & $0.991$ \\
		\textbf{Case~2} & $\bm{7.03}$ & $\bm{0.004}$ & --- & --- \\
		Case~3 & $5.27$ & $0.077$ & $-1.28$ & $0.713$ \\
		Case~4 & $3.73$ & $0.213$ & --- & --- \\
		Case~5 & $1.32$ & $0.898$ & $-1.53$ & $0.836$ \\
		\bottomrule
	\end{tabular}
	\par\vspace{0.3em}
	\footnotesize Exact $p$-values conditioned on $T = 65$. $t$-bounds test structurally undefined for Cases~2 and~4 (see text).
\end{table}
...
The $t$-bounds test never rejects. The strongest result (Case~3 with dummies: $t = -2.35$, $p = 0.269$) falls well short of conventional levels. The system cointegrates in the joint~$F$ sense but the speed at which output adjusts toward equilibrium is indistinguishable from zero. The split verdict---$F$ rejects, $t$ does not---indicates that the long-run relationship exists but the adjustment channel through output is either very slow or runs through a variable the bivariate system treats as exogenous.

% [R5] Reinterpretation of split verdict for CU series
The capacity utilization series implied by the ARDL is constructed from a cointegrating residual whose error-correction dynamics cannot be distinguished from zero---the ``equilibrium'' to which the series is referenced barely functions as an attractor within the sample.

The $t$-bounds test is structurally undefined for Cases~2 and~4. In these cases, the deterministic component (intercept or trend) is restricted to the long-run equation, entering the ECM multiplied by $\pi_y$. Under the null $H_0{:}\; \pi_y = 0$, the restricted deterministic vanishes from the equation---whether the model has an intercept depends on the parameter being tested. This circularity prevents PSS from tabulating critical values. The $F$-test escapes because the joint null wipes the entire levels block regardless of entanglement.


\begin{table}[!ht]
	\centering
	\small
	\caption{PSS bounds test: dummy toggle comparison. ARDL(1,3), Cases~I--V.}
	...
	\end{tabular}
	\par\vspace{0.3em}
	\footnotesize Exact $p$-values, $T=65$, $R=40{,}000$ draws. Removing dummies concentrates evidence into Case~2 alone; Case~3 weakens from $p=0.038$ to $p=0.077$.
\end{table}


\subsubsection{What Case~2 Means}
...
\begin{figure}[!ht]
	...
	\caption{S0--S1 closing figure: capacity utilization specification fan. 24 series from the full \{pairing $\times$ lag order $\times$ trend $\times$ dummy\} grid, faceted by capital stock concept. Left: Gross~$K$ (blue family)---tight band, tracks Shaikh's shape, diverges from FRB post-1990. Right: Net~$K$ (orange family)---wide band, drifts past 120\%, $\psi_t$ contamination visible. Benchmarks: Shaikh~$u_K$ (dark red), FRB (black).}
	\label{fig:S0S1_fan}
\end{figure}


% =========================================================================
% §1.4.5 — S2: System Estimation (stub)
% =========================================================================

\subsection{S2: System Estimation}
\label{subsec:S2_results}

\textit{[S2 results pending. Johansen rank test, VECM estimation, loading matrix extraction not yet run.]}
