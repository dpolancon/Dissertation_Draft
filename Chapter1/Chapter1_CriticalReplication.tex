...
% Consolidated draft v2 — 2026-03-16
% NOTE: Feasible ceiling paragraph (k→1) opens §1.3.2, not here.
% =============================================================================
...
% --- BLOCK 1: REPRODUCTION SCHEMAS AS ACCOUNTING FOUNDATION ---
...
% --- SHAIKH'S FORECLOSURE OF VOLUME II ---
...
% --- BLOCK 2: SMOOTH REPRODUCTION CLOSURE ---
...
% =========================================================================
% §3.2.2 — Unbalanced Growth Closure and Classical Investment Condition
% =========================================================================

\subsubsection{The Unbalanced Growth Closure and the Classical Investment Condition}
...
\end{equation}
%
an identity in which $\theta$ cancels completely. The closure enters through $\hat{b}$ and through $\hat{\phi}^p$ with opposite signs and zeros out. The three equations---the accounting identity, the closure, and the productivity consequence---are underdetermined for the dynamics of $\hat{k}$: the system has three unknowns ($\hat{k}$, $\hat{y}^p$, $\hat{\imath}$) and two independent relations.

The missing ingredient is a closure on investment. Under the Say's law abstraction ($\mu = 1$), the minimal classical closure is:
%
\begin{equation}\label{eq:classical_closure}
	\phi^p = s \qquad \text{(constant)}
\end{equation}
%
The share of productive capacity claimed by investment is structurally fixed. This is the first behavioral assumption in the system---the classical savings-determines-investment condition: under Say's law, the investment share is determined by the savings propensity of the surplus, with no demand-side feedback to alter it. With $\phi^p$ constant, $\hat{\phi}^p = 0$, which pins investment growth to capacity growth: $\hat{\imath} = \hat{y}^p = \theta\hat{k}$. The system closes and $\theta$-dependent dynamics emerge.

% =========================================================================
% §3.2.3 — Bernoulli ODE and Regime Classification
% =========================================================================

\subsubsection{The Bernoulli ODE and Regime Classification}

From $\hat{k} + \delta = sB$ with $s$ constant, differentiate:
%
\begin{equation}\label{eq:bernoulli_deriv}
	\frac{d\hat{k}}{dt} = s\dot{B} = sB\,\hat{b} = (\hat{k} + \delta)\,\hat{b}
\end{equation}
%
Substituting $\hat{b} = (\theta - 1)\hat{k}$:
%
\begin{equation}\label{eq:bernoulli}
	\boxed{\frac{d\hat{k}}{dt} = (\theta(\Lambda) - 1)\,\hat{k}\,(\hat{k} + \delta)}
\end{equation}
%
This is a Bernoulli equation in $\hat{k}$: nonlinear, but analytically solvable. The $\theta$-dependent dynamics emerge because the constant investment share prevents the investment channel from absorbing the $\theta$ signal---the regime's structural character feeds back directly into the acceleration of accumulation.
...
% =========================================================================
% §3.2.4 — The Mapping of Capital Productivity
% =========================================================================

\subsubsection{The Mapping of Capital Productivity}

From $\hat{b} = (\theta - 1)\hat{k}$ and the Bernoulli solution for $\hat{k}(t)$, the level trajectory of $B$ is recovered by integration:
%
...
The regime classification, the instability result, the capital productivity mapping, and the absence of a feasible interior attractor all emerge from three ingredients: the accounting identity $\hat{k} + \delta = \phi^p B$, the unbalanced growth closure $\hat{y}^p = \theta\hat{k}$, and the classical investment closure $\phi^p = s$. The first two are pre-behavioral; the third is the classical savings condition under Say's law---the only behavioral assumption in the system. No investment function, Phillips curve, or demand-formation equation has been imposed.

% =========================================================================
% §3.2.5 — The Extended Closure: Autonomous Technical Progress
% =========================================================================

\subsubsection{The Extended Closure: Autonomous Technical Progress and the Quadratic Law}
...
\paragraph{Vieta's formulas.} Writing the quadratic $(\theta - 1)\hat{k}^2 + [(\theta - 1)\delta + \beta]\hat{k} + \beta\delta = 0$, Vieta's formulas give:
%
...
\paragraph{The degeneracy locus.} As $\theta \to 1$, the quadratic coefficient vanishes and the equilibrium $\hat{k}_2^*$ diverges:
%
...
\paragraph{The pure closure as limiting case.} When $\beta \to 0$, $\hat{k}_2^* \to 0$, the positive equilibrium collapses to the stagnation point, the quadratic degenerates to $(\theta - 1)\hat{k}(\hat{k} + \delta) = 0$, and the Bernoulli ODE of \S\ref{sec:accounting_laws} is recovered. The pure closure is the limiting case of Shaikh's specification in which the entire capital-to-capacity mapping is exhausted by the mechanization choice. The quadratic structure and Vieta's formulas are properties of Shaikh's extended closure, not of the accounting laws per se.
...
The accounting laws of capital accumulation under unbalanced growth establish the following.

The \textbf{regime classification} is determined by the sign of $(\theta - 1)$. Over-mechanization ($\theta < 1$) generates decelerating accumulation; under-mechanization ($\theta > 1$) generates accelerating accumulation; balanced growth ($\theta = 1$) is the knife-edge singularity. This result requires the accounting identity, the pure closure, and the classical investment condition $\phi^p = s$---no further behavioral assumptions.
...
The \textbf{empirical task} is now defined. The transformation elasticity $\theta(\Lambda)$ must be recovered from observed data, and its regime-dependence tested. The next section develops Shaikh's identification strategy for this recovery, operating within his extended closure and the ARDL bounds-testing framework it implies.
